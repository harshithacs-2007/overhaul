...
describe("simulateBuildingHourly", () => {
  it("uses the complete hourly boundary", () => { const result = simulateBuildingHourly(base); expect(result.datasetHours).toBe(8760); expect(result.peakThermalLoadKW).toBeGreaterThan(0); expect(result.annualCoolingEnergyKWh).toBeGreaterThan(0); expect(result.annualCostINR).toBe(result.annualCoolingEnergyKWh * 8); expect(result.latentCoolingEnergyKWh).toBeGreaterThan(0); });
