...
import { useWorkspace, type WorkspaceSection } from "./WorkspaceProvider";
import { collectUnknowns } from "@/lib/engineering/model";

function valueOrUnknown(
  value: { value?: string | number | null; provenance?: { kind: string } } | null | undefined,
  suffix = ""
) {
  if (!value || value.provenance?.kind === "UNKNOWN" || value.value == null) {
    return "Unknown";
  }
...
export function OverviewPanel() {
  ...
  const openSection = (section: WorkspaceSection, label: string) => {
    setSection(section);
    setStatus(`${label} workspace opened`);
  };

  const pipeline = [
    ["Evidence", state.evidence.length ? "COLLECTED" : "EMPTY", "evidence" as WorkspaceSection],
    [
      "Building model",
      model.location || model.building.floorAreaM2.value != null ? "IN PROGRESS" : "START",
      "building" as WorkspaceSection,
    ],
    [
      "Climate",
      model.climate?.completeness.engineeringUsable ? "USABLE" : "INCOMPLETE",
      "climate" as WorkspaceSection,
    ],
    [
      "HVAC",
      hvac?.ratedCapacityKW.value != null ? "SPECIFIED" : "INCOMPLETE",
      "hvac" as WorkspaceSection,
    ],
    ["Physics", "READY TO RUN", "physics" as WorkspaceSection],
    ["Retrofit", "WHAT-IF", "simulate" as WorkspaceSection],
  ] as const;
    ...
    <div className="flex h-full flex-col overflow-y-auto bg-navy">
      <header className="border-b border-steel/20 px-5 py-5 lg:px-7">
        <p className="text-[10px] uppercase tracking-[0.2em] text-steel">
          OVERHAUL / BUILDING INTELLIGENCE
        </p>
        <div className="mt-2 flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm leading-relaxed text-steel">
              One model feeds evidence, climate, HVAC, physics and retrofit
              decisions. Unknowns stay explicit until supported.
            </p>
          </div>
      ...
      <div className="grid gap-px border-b border-steel/20 bg-steel/15 sm:grid-cols-2 xl:grid-cols-4">
        <Metric
          label="Location"
          value={model.location?.displayName ?? "Not resolved"}
          detail={model.location?.precision ?? "no location"}
        />
        <Metric
          label="Floor area"
          value={valueOrUnknown(model.building.floorAreaM2, " m²")}
          detail="building identity"
        />
        <Metric
          label="HVAC capacity"
          value={valueOrUnknown(hvac?.ratedCapacityKW, " kW")}
          detail={hvac?.systemType.value ?? "type unknown"}
        />
        <Metric
          label="Evidence"
          value={String(state.evidence.length)}
          detail="uploaded / captured"
        />
      </div>
        ...
        <section className="border border-steel/20 bg-navy/60 p-5">
          <div className="flex items-center justify-between gap-4">
            <div>
              ...
              <h2 className="font-display mt-1 text-xl text-paper">
                Evidence → decision
              </h2>
            </div>
          ...
          <div className="mt-5 grid gap-2 sm:grid-cols-2">
            {pipeline.map(([label, stateLabel, target]) => (
              <button
                key={label}
                type="button"
                onClick={() => openSection(target, label)}
        ...
        <section className="border border-gold/25 bg-gold/[0.03] p-5">
          ...
          <h2 className="font-display mt-1 text-2xl text-paper">
            {state.evidence.length === 0
              ? "Bring in evidence"
              : unknowns.length
                ? "Resolve the highest-impact unknowns"
                : "Run the engineering pipeline"}
          </h2>
          <p className="mt-3 text-sm leading-relaxed text-steel">
            OVERHAUL promotes a field to engineering calculations only once its
            source and value are explicit.
          </p>
          <div className="mt-5 flex flex-wrap gap-2">
            <button
              type="button"
              onClick={openEvidence}
              className="border border-teal/70 bg-teal/10 px-4 py-2 text-sm text-teal"
            >
              Open evidence
            </button>
            <button
              ...
              className="border border-steel/35 px-4 py-2 text-sm text-paper"
            >
              Edit model
            </button>
          </div>
...
function Metric({
  ...
  return (
    <div className="bg-navy p-4">
      ...
      <p className="mt-1 truncate font-mono-num text-[10px] text-steel/80">
        {detail}
      </p>
    </div>
  );
