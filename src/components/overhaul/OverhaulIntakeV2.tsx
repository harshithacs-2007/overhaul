...
export default function OverhaulIntakeV2() {
  ...
  const addFiles = (kind: EvidenceKind | null, files: File[]) => {
    ...
    const next = files
      ...
      .map((file) => ({
        id: uid(),
        kind: kind || inferKind(file),
        name: file.name || "Untitled evidence",
        type: file.type,
        size: file.size,
      ...
      <div className="flex min-h-screen">
        <aside className="hidden w-[240px] shrink-0 border-r border-steel/15 bg-[#070909] lg:flex lg:flex-col">
          ...
          <div className="border-y border-steel/10 px-4 py-5">
            <p className="px-2 font-mono text-[8px] uppercase tracking-[0.16em] text-steel">Current inspection</p>
            <div className="mt-3 border border-teal/25 bg-teal/[0.04] px-3 py-3"><p className="font-mono text-[8px] uppercase text-teal">{scopeLabel}</p><p className="mt-1 text-[10px] text-steel">Start with evidence. OVERHAUL fills nothing in by guesswork.</p></div>
          </div>
          <nav className="px-4 py-5">
            {[["01", "Observe"], ["02", "Understand"], ["03", "Simulate"], ["04", "Decide"]].map(([n, title], index) => <div key={n} className={`mb-1 border px-3 py-3 ${index === 0 ? "border-teal/20 bg-teal/[0.04]" : "border-transparent"}`}><div className="flex gap-3"><span className="font-mono text-[8px] text-teal">{n}</span><p className="text-[10px]">{title}</p></div></div>)}
          </nav>
        ...
        <section className="min-w-0 flex-1">
          <header className="flex items-center justify-between border-b border-steel/15 px-5 py-4 sm:px-8">
            <div><p className="font-mono text-[8px] uppercase tracking-[0.16em] text-teal">New assessment</p><h1 className="mt-1 font-display text-2xl sm:text-3xl">Start with what you have.</h1></div>
            <button type="button" onClick={() => fileInput.current?.click()} className="border border-paper/25 px-3 py-2 font-mono text-[8px] uppercase tracking-[0.12em] text-paper hover:border-teal hover:text-teal">+ Add evidence</button>
          </header>
          ...
          <div className="mx-auto max-w-[1450px] p-5 sm:p-8">
            <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_310px]">
              <div className="space-y-5">
                <div className="grid gap-3 md:grid-cols-3">
                  {(["building", "facility", "equipment"] as Scope[]).map((item) => (
                    <button key={item} type="button" onClick={() => setScope(item)} className={`relative border p-4 text-left transition ${scope === item ? "border-teal/55 bg-teal/[0.07]" : "border-steel/15 bg-[#080c0c] hover:border-steel/40"}`}>
                      <p className="font-mono text-[8px] uppercase tracking-[0.15em] text-teal">{item}</p>
                      <p className="mt-2 font-display text-2xl">{item === "building" ? "Building" : item === "facility" ? "Facility" : "Equipment"}</p>
                      <p className="mt-1 text-[9px] leading-4 text-steel">{item === "building" ? "Home, apartment, office, school, hospital" : item === "facility" ? "Plant, warehouse, cold store, campus" : "Chiller, pump, compressor, motor, boiler or other machine"}</p>
                      {scope === item ? <span className="mt-3 inline-block font-mono text-[7px] uppercase tracking-[0.12em] text-teal">Selected</span> : null}
                    </button>
                  ))}
                </div>

                <div className="border border-steel/15 bg-[#080c0c]">
                  <div className="border-b border-steel/10 px-4 py-4 sm:px-5">
                    <p className="font-mono text-[8px] uppercase tracking-[0.14em] text-teal">Step 1 · Evidence</p>
                    <h2 className="mt-1 font-display text-3xl">Show it. Don't describe it.</h2>
                    <p className="mt-1 max-w-2xl text-[10px] leading-5 text-steel">Upload whatever already exists: a photo, nameplate, bill, floor plan, drawing, manual, inspection report or meter export. One useful artifact is enough to start.</p>
                  </div>

                  <div className="grid gap-4 p-4 sm:p-5 lg:grid-cols-[1.15fr_0.85fr]">
                    <div className={`relative grid min-h-[370px] place-items-center overflow-hidden border bg-black/15 transition ${dragging ? "border-teal bg-teal/[0.04]" : "border-dashed border-steel/25"}`} onDragEnter={(e) => { e.preventDefault(); setDragging(true); }} onDragOver={(e) => e.preventDefault()} onDragLeave={() => setDragging(false)} onDrop={(e) => { e.preventDefault(); setDragging(false); addFiles(null, Array.from(e.dataTransfer.files)); }}>
                      <div className="absolute inset-0 opacity-25" style={{ backgroundImage: "linear-gradient(rgba(138,155,168,.10) 1px,transparent 1px),linear-gradient(90deg,rgba(138,155,168,.10) 1px,transparent 1px)", backgroundSize: "36px 36px" }} />
                      <div className="relative z-10 max-w-lg px-6 text-center">
                        <div className="mx-auto grid h-16 w-16 place-items-center rounded-full border border-teal/30 bg-teal/[0.04] font-mono text-xl text-teal">＋</div>
                    ...
                    <div className="flex min-h-[370px] flex-col border border-steel/10 bg-black/10 p-4">
                      <div className="flex items-center justify-between"><p className="font-mono text-[8px] uppercase tracking-[0.14em] text-steel">Evidence shelf</p><span className={`font-mono text-[8px] ${evidence.length ? "text-teal" : "text-steel"}`}>{evidence.length}/8</span></div>
                ...
                <div className="border border-steel/15 bg-[#080c0c]">
                  <button type="button" onClick={() => setDetailsOpen((value) => !value)} className="flex w-full items-center justify-between px-4 py-4 text-left sm:px-5"><div><p className="font-mono text-[8px] uppercase tracking-[0.14em] text-steel">Step 2 · Optional context</p><p className="mt-1 text-[10px] text-paper">Help OVERHAUL orient the evidence. Skip anything you don't know.</p></div><span className="font-mono text-[9px] text-teal">{detailsOpen ? "−" : "+"}</span></button>
                  {detailsOpen ? <div className="grid gap-4 border-t border-steel/10 p-4 sm:grid-cols-2 sm:p-5">
                    <label className="block"><span className="font-mono text-[7px] uppercase text-steel">What matters most?</span><div className="mt-2 grid gap-1.5">{goals.map(([value, label, sub]) => <button key={value} type="button" onClick={() => setGoal(value)} className={`border p-2.5 text-left ${goal === value ? "border-teal/35 bg-teal/[0.05]" : "border-steel/10"}`}><p className="text-[9px]">{label}</p><p className="mt-0.5 text-[7px] text-steel">{sub}</p></button>)}</div></label>
                    <div className="space-y-4">
                      <label className="block"><span className="font-mono text-[7px] uppercase text-steel">Site / asset name</span><input value={siteName} onChange={(e) => setSiteName(e.target.value)} placeholder="Optional" className="mt-2 w-full border-b border-steel/20 bg-transparent py-2 text-[10px] outline-none focus:border-teal" /></label>
                      <label className="block"><span className="font-mono text-[7px] uppercase text-steel">Industry</span><select value={industry} onChange={(e) => setIndustry(e.target.value as Industry)} className="mt-2 w-full border-b border-steel/20 bg-[#080c0c] py-2 text-[10px] outline-none focus:border-teal">{industries.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>
                      {scope === "equipment" ? <><label className="block"><span className="font-mono text-[7px] uppercase text-steel">Known machine class</span><select value={equipmentType} onChange={(e) => setEquipmentType(e.target.value)} className="mt-2 w-full border-b border-steel/20 bg-[#080c0c] py-2 text-[10px] outline-none focus:border-teal"><option value="">Let evidence identify it</option>{equipmentOptions.map((value) => <option key={value} value={value}>{value}</option>)}</select></label><label className="block"><span className="font-mono text-[7px] uppercase text-steel">Approximate age, if known</span><input inputMode="numeric" type="number" min="0" max="100" value={assetAge} onChange={(e) => setAssetAge(e.target.value)} placeholder="Optional" className="mt-2 w-full border-b border-steel/20 bg-transparent py-2 text-[10px] outline-none focus:border-teal" /></label></> : null}
                    </div>
                  </div> : null}
                </div>
              </div>
              ...
              <aside className="h-fit border border-steel/15 bg-[#080c0c] p-5 xl:sticky xl:top-5">
                <p className="font-mono text-[8px] uppercase tracking-[0.15em] text-teal">Before analysis</p>
                <h2 className="mt-1 font-display text-2xl">{scopeLabel}</h2>
                <p className="mt-1 text-[9px] leading-4 text-steel">{goal === "retrofit" ? "Find and compare justified retrofit interventions." : goals.find(([x]) => x === goal)?.[2]}</p>

                <div className="mt-5 border border-steel/10 p-4"><p className="font-mono text-[8px] uppercase tracking-[0.12em] text-steel">What happens next</p><div className="mt-3 space-y-2 text-[9px]"><p><span className="text-teal">01</span> Extract facts + provenance</p><p className="text-steel"><span className="text-teal">02</span> Build the engineering baseline</p><p className="text-steel"><span className="text-teal">03</span> Identify missing decision-critical evidence</p><p className="text-steel"><span className="text-teal">04</span> Simulate only supported interventions</p><p className="text-steel"><span className="text-teal">05</span> Rank the decision and verify it later</p></div></div>

                <div className="mt-5 border border-gold/20 bg-gold/[0.035] p-3"><p className="font-mono text-[7px] uppercase tracking-[0.12em] text-gold">Accuracy rule</p><p className="mt-2 text-[9px] leading-4 text-steel">Evidence values are kept traceable. Known unit conversions are deterministic. Unsupported values stay unknown and block calculations that would otherwise be guesses.</p></div>

                {analysisError ? <div className="mt-4 border border-clay/25 bg-clay/5 p-3 text-[9px] leading-4 text-clay">{analysisError}</div> : null}
                {analyzing ? <div className="mt-4 border border-teal/25 bg-teal/5 p-3"><p className="font-mono text-[8px] uppercase text-teal">Reading evidence</p><p className="mt-1 text-[9px] text-steel">{analysisCount}/{evidence.length} artifacts analyzed.</p><div className="mt-2 h-1 overflow-hidden bg-steel/10"><div className="h-full bg-teal transition-all" style={{ width: `${Math.round((analysisCount / Math.max(evidence.length, 1)) * 100)}%` }} /></div></div> : null}

                <button disabled={!evidence.length || analyzing} onClick={analyzeEvidence} type="button" className="mt-5 w-full border border-teal bg-teal px-4 py-3 font-mono text-[8px] uppercase tracking-[0.13em] text-navy disabled:cursor-not-allowed disabled:opacity-25">{analyzing ? `Inspecting ${analysisCount}/${evidence.length}…` : "Analyze & build assessment"}</button>
                <p className="mt-3 text-center text-[8px] leading-4 text-steel">No “95% ready” meter. Readiness is established from the evidence and model completeness, not the number of files uploaded.</p>
              </aside>
            </div>
          </div>
