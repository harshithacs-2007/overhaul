...
export default function AssetTwinViewport({ scope, title, assetClass, evidenceIds, values }: Props) {
  ...
  const dimensionsEstablished = [parsedWidth, parsedDepth, parsedHeight].every((x) => Number.isFinite(x) && x > 0);
  const width = dimensionsEstablished ? parsedWidth : 0;
  const depth = dimensionsEstablished ? parsedDepth : 0;
  const height = dimensionsEstablished ? parsedHeight : 0;
  ...
  const exportBlocked = !dimensionsEstablished;
    ...
    <section className="border border-steel/15 bg-[#070b0b]">
      <div className="flex flex-wrap items-start justify-between gap-4 border-b border-steel/10 px-5 py-4">
        <div>
          ...
          <p className="mt-1 max-w-2xl text-[10px] leading-5 text-steel">Geometry is metric only when width, depth and height are evidenced or explicitly supplied. Otherwise OVERHAUL keeps the view schematic rather than inventing dimensions.</p>
        </div>
        <div className="flex flex-wrap gap-2">
          <button disabled={exportBlocked} type="button" onClick={() => downloadText(`${title.replace(/\W+/g, "-")}.obj`, buildAssetObj(asset), "text/plain;charset=utf-8")} className={`border px-3 py-2 font-mono text-[8px] uppercase ${exportBlocked ? "cursor-not-allowed border-steel/10 text-steel/50" : "border-teal/35 text-teal hover:bg-teal/5"}`}>Export OBJ · Blender</button>
          <button disabled={exportBlocked} type="button" onClick={() => downloadText(`${title.replace(/\W+/g, "-")}.dxf`, buildDxfFootprint(asset), "application/dxf")} className={`border px-3 py-2 font-mono text-[8px] uppercase ${exportBlocked ? "cursor-not-allowed border-steel/10 text-steel/50" : "border-steel/25 text-paper hover:border-teal"}`}>Export DXF · CAD</button>
        ...
        <div className="min-h-[420px] overflow-hidden border-b border-steel/10 xl:border-b-0 xl:border-r">
          <div className="relative h-full min-h-[420px] bg-[#050808]">
            ...
            <div className={`absolute left-1/2 top-[54%] h-[250px] w-[360px] -translate-x-1/2 -translate-y-1/2 [transform:skewY(-7deg)_rotateX(54deg)] border ${dimensionsEstablished ? "border-teal/50 bg-teal/[0.06] shadow-[0_0_60px_rgba(70,220,200,0.06)]" : "border-steel/30 bg-steel/[0.03]"}`}>
              ...
              {!dimensionsEstablished && <div className="absolute inset-0 grid place-items-center bg-black/35 text-center"><div><p className="font-mono text-[8px] uppercase tracking-[0.14em] text-amber-200">Geometry unresolved</p><p className="mt-2 px-8 text-[10px] text-steel">Add width × depth × height evidence to unlock metric CAD/Blender export.</p></div></div>}
            </div>
