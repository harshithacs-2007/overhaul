...
function canonical(field: string): string {
  return field.trim().toLowerCase().replace(/[^a-z0-9]+/g, "_").replace(/^_|_$/g, "");
}
      ...
      <div className="border border-steel/20 bg-black/15 p-5">
        ...
        {result.status === "insufficient-evidence" ? (
          <div className="mt-5 border border-clay/25 bg-clay/5 p-4">
            <p className="font-mono text-[9px] uppercase text-clay">Decision blocked</p>
            <div className="mt-2 space-y-1 text-xs leading-5 text-steel">{result.evidenceRequests.map((item) => <p key={item}>· {item}</p>)}</div>
          </div>
        ) : null}
        ...
        {result.candidates.length ? (
          <div className="mt-5 space-y-3">
            {result.candidates.map((candidate) => (
              <article key={candidate.causeId} className="border border-steel/15 p-4">
                <div className="flex flex-col gap-2 md:flex-row md:items-start md:justify-between">
                  <div>
                    <p className="font-mono text-[8px] uppercase tracking-[0.12em] text-teal">{candidate.causeId}</p>
                    <h3 className="mt-1 text-lg">{candidate.cause}</h3>
                  </div>
                  <span className={`font-mono text-[8px] uppercase ${candidate.severity === "critical" ? "text-clay" : candidate.severity === "warning" ? "text-amber-300" : "text-teal"}`}>{candidate.severity}</span>
                </div>
                <p className="mt-2 text-xs leading-5 text-steel">{candidate.consequence}</p>
                <div className="mt-4 grid gap-3 md:grid-cols-3">
                  <Block title="Supporting signals">{candidate.supportingSignals.join(" · ")}</Block>
                  <Block title="Discriminating evidence">{candidate.discriminatingEvidence.join(" · ")}</Block>
                  <Block title="Candidate actions">{candidate.recommendedActionIds.join(" · ")}</Block>
                </div>
              </article>
            ))}
          </div>
        ...
        {result.evidenceRequests.length ? (
          <div className="mt-5 border border-teal/20 p-4">
            <p className="font-mono text-[9px] uppercase tracking-[0.12em] text-teal">Next best measurement</p>
            <p className="mt-2 text-xs leading-5 text-steel">{result.evidenceRequests[0]}</p>
          </div>
        ) : null}
      </div>
    </section>
...
function Block({ title, children }: { title: string; children: string }) {
  return <div className="border border-steel/15 p-3"><p className="font-mono text-[8px] uppercase text-steel">{title}</p><p className="mt-2 text-xs leading-5 text-paper">{children}</p></div>;
}
