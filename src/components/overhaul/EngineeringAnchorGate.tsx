...
const BUILDING_FIELDS: Field[] = [
  ...
  { key: "solar_gain_factor", label: "Effective solar gain factor", unit: "m²", required: true, min: 0, step: "0.01" },
...
function readNumber(values: Values, key: string) {
...
export default function EngineeringAnchorGate({ scope, values, assetAgeYears }: { scope: Scope; values: Values; assetAgeYears?: number | null }) {
  ...
  useEffect(() => {
    const next = Object.fromEntries(fields.map((field) => [field.key, asString(values[field.key])]));
    if (scope === "equipment" && !next.asset_age_years && assetAgeYears != null) next.asset_age_years = String(assetAgeYears);
    setDraft(next);
  }, [fields, scope, values, assetAgeYears]);

  const missing = useMemo(() => fields.filter((field) => {
    if (!field.required) return false;
    const raw = draft[field.key]?.trim();
    if (!raw) return true;
    const parsed = Number(raw);
    if (!Number.isFinite(parsed)) return true;
    if (field.min != null && parsed < field.min) return true;
    if (field.max != null && parsed > field.max) return true;
    return false;
  }).map((field) => field.label), [draft, fields]);

  const equipmentOperatingAnchor = scope !== "equipment" || (
    readNumber({ load_kw: draft.load_kw, power_kw: draft.power_kw }, "load_kw") != null &&
    readNumber({ load_kw: draft.load_kw, power_kw: draft.power_kw }, "power_kw") != null
  ) || (
    Number.isFinite(Number(draft.load_kw)) && Number.isFinite(Number(draft.efficiency))
  );
  ...
  const apply = () => {
    ...
    try {
      ...
      if (scope === "equipment" && next.asset_age_years != null) {
        const assessment = JSON.parse(sessionStorage.getItem("overhaul:assessment") || "null") as Record<string, unknown> | null;
        if (assessment) sessionStorage.setItem("overhaul:assessment", JSON.stringify({ ...assessment, assetAgeYears: Number(next.asset_age_years) }));
      }
  ...
  return <section className={`border p-5 ${complete ? "border-teal/15 bg-teal/[.02]" : "border-amber-200/25 bg-amber-200/[.02]"}`}>
    <div className="flex flex-wrap items-start justify-between gap-4">
      <div>
        <p className="font-mono text-[8px] uppercase tracking-[.18em] text-amber-200">Engineering anchor gate</p>
        <h2 className="mt-1 font-display text-2xl">No guessed engineering numbers.</h2>
        <p className="mt-2 max-w-3xl text-[9px] leading-5 text-steel">Material/asset age is explicit context. Thermal properties and operating state must still come from evidence or a user-supplied measurement. A visual estimate cannot become a design input.</p>
      </div>
      <div className={`border px-3 py-2 font-mono text-[8px] uppercase ${complete ? "border-teal/25 text-teal" : "border-amber-200/25 text-amber-200"}`}>{complete ? "Inputs complete" : `${missing.length + (equipmentOperatingAnchor ? 0 : 1)} anchor(s) missing`}</div>
    </div>
    <div className="mt-4 grid gap-2 md:grid-cols-2 xl:grid-cols-3">
      {fields.map((field) => <label key={field.key} className="block border border-steel/10 bg-black/15 p-3"><span className="flex items-center justify-between gap-3 font-mono text-[7px] uppercase tracking-[.08em] text-steel"><span>{field.label}{field.required ? <b className="ml-1 text-amber-200">*</b> : null}</span><span>{field.unit}</span></span><input value={draft[field.key] || ""} type="number" min={field.min} max={field.max} step={field.step || "any"} onChange={(event) => setDraft((current) => ({ ...current, [field.key]: event.target.value }))} className="mt-2 w-full border border-steel/15 bg-[#050707] px-3 py-2 text-[11px] text-paper outline-none focus:border-teal/45" /></label>)}
    </div>
    ...
    {scope !== "equipment" ? <p className="mt-3 text-[8px] leading-4 text-steel">The live building calculation will use the supplied envelope conductance, explicit air exchange, internal gain, and a real hourly NASA POWER outdoor-temperature/solar series. Material age selects context and evidence requirements; it never invents a U-value.</p> : null}
