...
export default function FinalReportView() {
  ...
  const report = useMemo(() => buildReport(scope, values, extracts, climate), [scope, values, extracts, climate]);
  ...
  return <main className="report-shell min-h-screen bg-[#f5f4ef] text-[#141817]">
    ...
    <article className="mx-auto max-w-[1180px] space-y-7 px-4 py-7 sm:px-7 sm:py-10">
      ...
      <Section title="05 · Climate & regional lens" kicker="never silently guessed"><div className="grid gap-4 md:grid-cols-[.8fr_1.2fr]"><div className="border border-[#d8d4ca] bg-white p-5"><p className="font-mono text-[7px] uppercase text-[#727b76]">Region supplied</p><p className="mt-2 text-xl">{region}</p><p className="mt-4 font-mono text-[8px] uppercase text-[#727b76]">Boundary evidence</p><p className="mt-1 font-mono text-sm">{report.climateBoundary}</p></div><div className="border border-[#d8d4ca] bg-white p-5"><p className="text-sm leading-6">Regional climate is treated as operating context, not a decoration. Explicit design or observed conditions remain the engineering baseline. Live regional weather is used only as a separate contextual stress condition; it never overwrites evidence or creates a savings multiplier.</p><div className="mt-4 grid gap-3 sm:grid-cols-2">{report.climateSignals.map((signal) => <div key={signal.label} className="border border-[#ebe7de] bg-[#faf9f5] p-3"><p className="font-mono text-[7px] uppercase text-[#727b76]">{signal.label}</p><p className="mt-1 font-mono text-sm">{signal.value}</p></div>)}</div><div className="mt-4 grid gap-3 sm:grid-cols-3">{[['Live temperature', climate?.temperature, '°C'], ['Live humidity', climate?.humidity, '% RH'], ['7-day range', climate?.min != null && climate?.max != null ? `${fmt(climate.min, 0)}–${fmt(climate.max, 0)}` : null, '°C']].map(([label, value, suffix]) => <div key={String(label)} className="border border-[#ebe7de] bg-[#faf9f5] p-3"><p className="font-mono text-[7px] uppercase text-[#727b76]">{label}</p><p className="mt-1 font-mono text-sm">{value == null ? 'Not resolved' : `${value}${suffix}`}</p></div>)}</div>{climate?.source ? <p className="mt-4 font-mono text-[7px] leading-4 text-[#7c847f]">Source: {climate.source}{climate.fetchedAt ? ` · refreshed ${new Date(climate.fetchedAt).toLocaleString('en-IN')}` : ''}</p> : null}</div></div></Section>

      <Section title="06 · Stress test" kicker="does the intervention hold?">{report.stress.length ? <div className="border border-[#d8d4ca] bg-white p-5"><StressGraph rows={report.stress}/><p className="mt-4 text-xs leading-5 text-[#656d68]">Each point is a rerun of the deterministic model under an explicit operating condition. Regional weather context is labeled separately when included; it is not a measured forecast.</p></div> : <div className="border border-[#d8d4ca] bg-white p-5 text-sm text-[#656d68]">Stress testing is not quantified yet because the evidence does not establish the target retrofit parameter.</div>}</Section>
      ...
      <Section title="08 · Decision & economics" kicker="what is justified"> <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{[['Annual energy reduction', `${report.energySaving > 0 ? fmt(report.energySaving) : '—'} kWh/yr`], ['Annual cost reduction', money(report.costSaving)], ['Simple payback', report.payback], ['Decision state', report.decisionState]].map(([label, value]) => <div key={label} className="border border-[#d8d4ca] bg-white p-4"><p className="font-mono text-[7px] uppercase text-[#727b76]">{label}</p><p className="mt-2 font-mono text-lg">{value}</p></div>)}</div><p className="mt-4 text-xs leading-5 text-[#656d68]">Installed cost, downtime and carbon are reported only when explicitly supplied or independently established. OVERHAUL will never display a zero-value tariff as zero savings; missing tariffs remain unquantified.</p></Section>
...
function buildReport(scope: Scope, values: Values, extracts: Extraction[], climate: ClimateContext) {
  ...
  for (const extraction of extracts) {
    for (const observation of extraction.observations || []) {
      evidence.push({
        source: extraction.sourceName || extraction.evidenceId || "Evidence source",
        field: observation.field.replaceAll('_', ' '),
        value: observation.numericValue != null && observation.unit ? `${fmt(observation.numericValue, 3)} ${observation.unit}` : observation.value,
  ...
  if (scope === "equipment" && load != null && capacity != null && efficiency != null && hours != null) {
    ...
    if (proposedEfficiency != null && proposedEfficiency > 0 && proposedEfficiency !== efficiency) {
      ...
      proposedEnergy = r.proposed.annualEnergyKWh; proposedPower = r.proposed.electricalPowerKW; energySaving = -r.delta.annualEnergyKWh; costSaving = rate != null ? r.delta.annualSavingINR : null;
  ...
  const stress = scope === "equipment" && load != null && capacity != null && efficiency != null && hours != null && proposedEfficiency != null && proposedEfficiency !== efficiency ? [0.25,0.5,0.75,1].map(f => { const l = capacity * f; const b = simulatePhysicsScenario({ subject:"equipment", baseline:{loadKW:l,ratedCapacityKW:capacity,efficiency,annualHours:hours,electricityRateINRPerKWh:rate??0} }); const r=simulatePhysicsScenario({ subject:"equipment",baseline:{loadKW:l,ratedCapacityKW:capacity,efficiency,annualHours:hours,electricityRateINRPerKWh:rate??0},retrofit:{efficiency:proposedEfficiency}}); return { label:`${Math.round(f*100)}%`,baseline:b.baseline.electricalPowerKW,proposed:r.proposed.electricalPowerKW,contextual:false }; }) : area != null && ua != null && outdoor != null && indoor != null && capacity != null && cop != null && coolingHours != null && proposedR != null && existingR != null && proposedR !== existingR ? [-2,0,2,4,6].map(d => { const temp=outdoor+d; const b={floorAreaM2:area,envelopeUA_W_per_K:ua,ventilationM3s:0,outdoorTempC:temp,indoorTempC:indoor,solarGainKW:0,internalGainKW:0,hvacCapacityKW:capacity,hvacCOP:cop,annualCoolingHours:coolingHours,electricityRateINRPerKWh:rate??0}; const current=simulatePhysicsScenario({subject:scope==='facility'?'facility':'building',baseline:b}); const r=simulatePhysicsScenario({subject:scope==='facility'?'facility':'building',baseline:b,retrofit:{envelopeUA_W_per_K:area/proposedR}}); return {label:`${temp.toFixed(0)}°C`,baseline:current.baseline.electricalPowerKW,proposed:r.proposed.electricalPowerKW,contextual:false}; }) : [];

  if ((scope === "building" || scope === "facility") && stress.length && Number.isFinite(Number(climate?.max)) && Math.abs(Number(climate?.max) - Number(outdoor)) >= 0.5) {
    const regionalHigh = Number(climate?.max);
    const regionalBaseline = { floorAreaM2: area!, envelopeUA_W_per_K: ua!, ventilationM3s: 0, outdoorTempC: regionalHigh, indoorTempC: indoor!, solarGainKW: 0, internalGainKW: 0, hvacCapacityKW: capacity!, hvacCOP: cop!, annualCoolingHours: coolingHours!, electricityRateINRPerKWh: rate ?? 0 };
    const current = simulatePhysicsScenario({ subject: scope === "facility" ? "facility" : "building", baseline: regionalBaseline });
    const retrofit = simulatePhysicsScenario({ subject: scope === "facility" ? "facility" : "building", baseline: regionalBaseline, retrofit: { envelopeUA_W_per_K: area! / proposedR! } });
    stress.push({ label: `Regional 7d high · ${regionalHigh.toFixed(1)}°C`, baseline: current.baseline.electricalPowerKW, proposed: retrofit.proposed.electricalPowerKW, contextual: true });
  }
  ...
  if (proposedEfficiency == null && proposedR == null && proposedHours == null) gaps.push('No explicit retrofit target has been supplied yet.');
  if (scope !== 'equipment' && !n(values, 'ventilation_m3s') && !n(values, 'solar_gain_kw') && !n(values, 'internal_gain_kw')) gaps.push('Ventilation, solar and internal gains are not established; the current building thermal model is partial.');
  ...
  const cards = [{label:'Modeled energy change',value: proposedEnergy != null ? `${fmt(saving)} kWh/yr` : 'Not quantified'},{label:'Modeled cost change',value: costSaving != null ? money(costSaving) : 'Not quantified'},{label:'Evidence observations',value:`${evidence.length}` }];
  return { baselineEnergy, proposedEnergy, baselinePower, proposedPower, energySaving: energySaving ?? 0, costSaving, payback: 'Requires installed cost', decisionState: energySaving && energySaving > 0 ? 'Quantified pathway' : 'Evidence-gated', recommendation: rec, cards, metrics, evidence, pathways: energyRows, climateSignals, climateBoundary: outdoor != null ? `${fmt(outdoor)} °C stated boundary` : 'Unresolved', gaps, stress };
}
...
function StressGraph({ rows }: { rows: Array<{ label:string; baseline:number; proposed:number; contextual?: boolean }> }) { const max=Math.max(...rows.flatMap(r=>[r.baseline,r.proposed]),1); return <div><div className="grid h-64 items-end gap-2" style={{gridTemplateColumns:`repeat(${rows.length}, minmax(0,1fr))`}}>{rows.map(row=><div key={row.label} className={`flex h-full items-end justify-center gap-1 border-b border-[#e1ddd3] ${row.contextual ? 'bg-[#fbf4df]' : ''}`}> <div className="w-1/3 bg-[#aeb7b1]" style={{height:`${Math.max(4,row.baseline/max*100)}%`}}/><div className="w-1/3 bg-[#7b6a3e]" style={{height:`${Math.max(4,row.proposed/max*100)}%`}}/></div>)}</div><div className="mt-3 grid gap-2" style={{gridTemplateColumns:`repeat(${rows.length}, minmax(0,1fr))`}}>{rows.map(row=><div key={row.label} className={`text-center font-mono text-[8px] ${row.contextual ? 'text-[#7b6a3e]' : 'text-[#707974]'}`}>{row.label}</div>)}</div><div className="mt-4 flex flex-wrap justify-center gap-5 font-mono text-[8px] uppercase text-[#707974]"><span>■ Current</span><span className="text-[#7b6a3e]">■ Retrofit</span><span className="text-[#7b6a3e]">Regional context ≠ design baseline</span></div></div>; }
