...
export default function RetrofitCommandCenter() {
  ...
  const shadow = makeShadow(metrics, heatStress);
...
function makeShadow(m:Record<string,number|null>,stress:number){const eLoad=m.area!=null?m.area*.1*(1+stress/100):null;const ePower=m.load!=null?m.load/(m.efficiency||3.4):null;const eCap=m.load!=null?m.load*1.1:null;return [sig("Indoor temperature",m.indoor,m.indoor!=null?24:null,"°C","comfort reference"),sig("Cooling load",m.load,eLoad,"kW","envelope + climate reference"),sig("Input power",m.power,ePower,"kW","load / efficiency reference"),sig("Installed capacity",m.capacity,eCap,"kW","load + reserve reference")]}
...
function canonical(field:string){const x=field.toLowerCase().replaceAll(" ","_");if(x.includes("floor")&&x.includes("area"))return"floor_area_m2";if(x.includes("rated")&&x.includes("capacity"))return"rated_capacity_kw";if(x.includes("capacity")&&x.includes("kw"))return"capacity_kw";if(x.includes("power"))return"power_kw";if(x.includes("cooling")&&x.includes("load"))return"cooling_load_kw";if(x.includes("hvac")&&x.includes("load"))return"hvac_load_kw";if(x.includes("indoor")&&x.includes("temp"))return"indoor_temp_c";if(x.includes("outdoor")&&x.includes("temp"))return"outdoor_temp_c";if(x.includes("temperature")&&x.includes("c"))return"temperature_c";if(x.includes("annual")&&x.includes("cool"))return"annual_cooling_hours";if(x.includes("annual")&&x.includes("runtime"))return"annual_runtime_hours";if(x.includes("runtime")&&x.includes("baseline"))return"baseline_runtime_hours";if(x.includes("runtime")&&x.includes("proposed"))return"proposed_runtime_hours";if(x.includes("efficiency"))return"efficiency";if(x.includes("cop"))return"cop";if(x.includes("electricity")&&x.includes("rate"))return"electricity_rate_inr_per_kwh";return x}
