...
export default function RetrofitPathfinder({ scope, values }: { scope: Scope; values: Values }) {
  ...
  const pathways = useMemo<Pathway[]>(() => {
    ...
    if (scope === "equipment") {
      ...
      const baselineReady = load != null && capacity != null && efficiency != null && hours != null;
      const candidates: Pathway[] = [];
      const efficiencyMissing = listMissing([["operating load", load != null], ["rated capacity", capacity != null], ["baseline efficiency", efficiency != null], ["annual runtime", hours != null], ["proposed efficiency", targetEfficiency != null]]);
      if (baselineReady && targetEfficiency != null && targetEfficiency > 0 && targetEfficiency !== efficiency) {
        const baseline = { loadKW: load!, ratedCapacityKW: capacity!, efficiency: efficiency!, annualHours: hours!, electricityRateINRPerKWh: rate ?? 0 };
      ...
      const runtimeMissing = listMissing([["current annual runtime", currentHours != null], ["target runtime", targetHours != null], ["operating load", load != null], ["rated capacity", capacity != null], ["efficiency", efficiency != null]]);
      if (currentHours != null && load != null && capacity != null && efficiency != null && targetHours != null && targetHours !== currentHours) {
        const baseline = { loadKW: load, ratedCapacityKW: capacity, efficiency, annualHours: currentHours, electricityRateINRPerKWh: rate ?? 0 };
    ...
    const hvacMissing = listMissing([["floor area", floorArea != null], ["envelope UA", ua != null], ["indoor/outdoor boundary", Number.isFinite(outdoor) && Number.isFinite(indoor)], ["HVAC capacity", capacity != null], ["baseline COP / efficiency", cop != null], ["annual cooling hours", hours != null], ["proposed HVAC efficiency", proposedEfficiency != null]]);
  ...
  const tariff = n(values, "electricity_rate_inr_per_kwh", "electricity_rate", "tariff_inr_per_kwh");
  const cost = Number(installedCost);
  const annualSaving = selectedPathway?.result?.delta.annualSavingINR ?? null;
  const simplePayback = tariff != null && annualSaving != null && annualSaving > 0 && Number.isFinite(cost) && cost > 0 ? cost / annualSaving : null;
