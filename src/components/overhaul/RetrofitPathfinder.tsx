...
export default function RetrofitPathfinder({ scope, values }: { scope: Scope; values: Values }) {
  ...
  const pathways = useMemo<Pathway[]>(() => {
    ...
    if (scope === "equipment") {
      ...
      if (baselineReady && proposedEfficiency != null && proposedEfficiency > 0 && proposedEfficiency !== efficiency) {
        ...
        candidates.push({
          ...
          gate: "Baseline and proposed equipment performance are established from supplied evidence.",
          computable: true,
        ...
        candidates.push({
          id: "efficiency",
          title: "Efficiency upgrade",
          mechanism: "Compare the installed equipment with a higher-efficiency replacement or upgrade.",
          gate: efficiencyMissing.length ? `Needs: ${efficiencyMissing.join(" · ")}.` : "Proposed efficiency must differ from the baseline.",
          computable: false,
        });
      }
      ...
      const runtimeMissing = listMissing([
        ...
        ["target runtime or control schedule", proposedHours != null],
        ["operating load", load != null],
        ["rated capacity", capacity != null],
      ...
      if (currentHours != null && load != null && capacity != null && efficiency != null && rate != null && proposedHours != null && proposedHours !== currentHours) {
        ...
        candidates.push({
          ...
          computable: true,
          result: simulatePhysicsScenario({ subject: "equipment", baseline, retrofit: { annualHours: proposedHours } }),
        });
      } else {
        candidates.push({
          id: "runtime",
          title: "Runtime / controls optimisation",
          mechanism: "Reduce unnecessary operating hours without assuming an efficiency change.",
          gate: runtimeMissing.length ? `Needs: ${runtimeMissing.join(" · ")}.` : "Proposed runtime must differ from the baseline.",
          computable: false,
        });
      }
      ...
      candidates.push({
        ...
        gate: "Needs measured/reference evidence such as power, flow, pressure, temperature, vibration, or a documented fault condition.",
        computable: false,
      });
      return candidates;
    }
    ...
    const envelopeMissing = listMissing([
      ["floor area", floorArea != null],
      ["baseline envelope UA", ua != null],
      ["outdoor design condition", Number.isFinite(outdoor)],
      ["indoor target condition", Number.isFinite(indoor)],
      ["HVAC capacity", capacity != null],
      ["HVAC COP / efficiency", cop != null],
      ["annual cooling hours", hours != null],
      ["electricity rate", rate != null],
      ["existing R-value", currentR != null],
      ["proposed R-value", proposedR != null],
    ]);
    ...
    if (baselineReady && proposedR != null && proposedR > 0 && currentR != null && proposedR !== currentR) {
      ...
      candidates.push({
        ...
        gate: "Floor area, envelope UA, indoor/outdoor boundary, HVAC performance, cooling hours and both R-values are established.",
        computable: true,
      ...
      candidates.push({
        id: "envelope",
        title: "Envelope retrofit",
        mechanism: "Model load reduction from insulation/envelope improvements before sizing HVAC consequences.",
        gate: envelopeMissing.length ? `Needs: ${envelopeMissing.join(" · ")}.` : "The proposed R-value must differ from the existing R-value.",
        computable: false,
      });
    }
    ...
    const hvacMissing = listMissing([
      ["floor area", floorArea != null],
      ["envelope UA", ua != null],
      ["indoor/outdoor boundary", Number.isFinite(outdoor) && Number.isFinite(indoor)],
      ["HVAC capacity", capacity != null],
      ["baseline COP / efficiency", cop != null],
      ["annual cooling hours", hours != null],
      ["electricity rate", rate != null],
      ["proposed HVAC efficiency", proposedEfficiency != null],
    ]);
    if (baselineReady && proposedEfficiency != null && proposedEfficiency > 0 && proposedEfficiency !== cop) {
      ...
      candidates.push({
        ...
        gate: "Baseline HVAC model and proposed efficiency are established.",
        computable: true,
      ...
      candidates.push({
        id: "hvac-efficiency",
        title: "HVAC efficiency upgrade",
        mechanism: "Evaluate a better-performing HVAC option without guessing its COP.",
        gate: hvacMissing.length ? `Needs: ${hvacMissing.join(" · ")}.` : "Proposed efficiency must differ from the baseline.",
        computable: false,
      });
    }
    ...
    candidates.push({
      ...
      mechanism: "Use actual operating schedules, setpoints, staging, and part-load behavior to reduce avoidable operation.",
      gate: "Needs operating-hour, setpoint, BMS trend, or controller evidence before quantification.",
      computable: false,
    });
    candidates.push({
      id: "capacity",
      ...
      gate: "Needs credible peak/design boundary and load inputs; resizing is not treated as automatic energy saving.",
      computable: false,
    });
    return candidates;
  }, [scope, values]);
      ...
      <div className="border-b border-steel/10 px-5 py-5">
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <p className="mt-2 max-w-3xl text-[10px] leading-5 text-steel">Retrofit pathways stay visible even when the model is incomplete. Quantified pathways run through the deterministic engineering model; blocked pathways tell the user exactly what evidence would unlock them.</p>
          </div>
      ...
      {selectedPathway ? (
        <div className="grid xl:grid-cols-[.85fr_1.15fr]">
          ...
          <div className="p-5">
            ...
            {selectedPathway.result ? <div className="mt-4 border border-teal/15 bg-teal/[0.025] p-4"><p className="font-mono text-[8px] uppercase text-teal">Why this number exists</p><div className="mt-2 space-y-1 text-[9px] leading-5 text-steel">{selectedPathway.result.verdict.length ? selectedPathway.result.verdict.map((line) => <p key={line}>{line}</p>) : <p>Computed from supplied baseline and intervention parameters only.</p>}</div></div> : <div className="mt-5 border border-steel/15 bg-black/20 p-4"><p className="font-mono text-[8px] uppercase text-steel">Evidence gate</p><p className="mt-2 text-[9px] leading-5 text-steel">This pathway may be relevant, but OVERHAUL will not attach a savings value until the listed engineering evidence is available.</p></div>}
          </div>
        </div>
      ) : null}
    </section>
