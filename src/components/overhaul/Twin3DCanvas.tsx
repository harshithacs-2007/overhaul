...
export default function Twin3DCanvas({ model, title }: Props) {
  ...
  useEffect(() => {
    ...
    const mount = async () => {
      ...
      scene.add(new THREE.HemisphereLight(0xd7fff8, 0x0a1111, 1.8)); const key = new THREE.DirectionalLight(0xffffff, 2.6); key.position.set(12, 18, 10); key.castShadow = true; scene.add(key); const rim = new THREE.PointLight(0x24dfca, 10, 60); rim.position.set(-10, 8, -12); scene.add(rim); scene.add(new THREE.GridHelper(Math.max(model.overall.widthM, model.overall.depthM) * 3, 60, 0x164b46, 0x0b2523));
      ...
      for (const room of model.rooms) { box(room.widthM, .03, room.depthM, materials.floor, room.x + room.widthM / 2, 0, room.y + room.depthM / 2); const roof = box(room.widthM, .015, room.depthM, materials.wire, room.x + room.widthM / 2, room.heightM, room.y + room.depthM / 2); roof.userData.type = "room-roof"; }
      for (const wall of model.walls) { const dx = wall.b.x - wall.a.x; const dz = wall.b.y - wall.a.y; const length = Math.hypot(dx, dz); if (!(length > 0)) continue; const mesh = box(length, wall.heightM, wall.thicknessM, wall.source === "inferred" ? materials.inferred : materials.wall, (wall.a.x + wall.b.x) / 2, 0, (wall.a.y + wall.b.y) / 2); mesh.rotation.y = Math.atan2(dz, dx); }
      for (const opening of model.openings) { const marker = new THREE.Mesh(new THREE.BoxGeometry(opening.widthM, .06, Math.max(model.overall.widthM, model.overall.depthM) * .012), materials.opening); marker.position.set(opening.x, .03, opening.y); root.add(marker); }
      ...
      const maxDim = Math.max(model.overall.widthM, model.overall.depthM, model.overall.heightM); const target: Vector3 = new THREE.Vector3(model.overall.widthM / 2, model.overall.heightM * .42, model.overall.depthM / 2); let yaw = .72, pitch = 1, radius = Math.max(maxDim * 1.9, 3), dragging = false, lastX = 0, lastY = 0;
