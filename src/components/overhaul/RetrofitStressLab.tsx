...
export default function RetrofitStressLab({ scope, values, climate = null }: { scope: Scope; values: Values; climate?: ClimateContext }) {
  const rows = useMemo<Row[]>(() => {
    ...
    if (Number.isFinite(regionalHigh) && regionalHigh > 0 && Math.abs(regionalHigh - outdoor) >= 0.5) {
      const baseline = { floorAreaM2: area, envelopeUA_W_per_K: ua, ventilationM3s: 0, outdoorTempC: regionalHigh, indoorTempC: indoor, solarGainKW: 0, internalGainKW: 0, hvacCapacityKW: capacity, hvacCOP: cop, annualCoolingHours: hours, electricityRateINRPerKWh: rate ?? 0 };
      const current = simulatePhysicsScenario({ subject: scope === "facility" ? "facility" : "building", baseline });
      const retrofit = simulatePhysicsScenario({ subject: scope === "facility" ? "facility" : "building", baseline, retrofit: { envelopeUA_W_per_K: proposedUA } });
  ...
  if (!rows.length) {
    return <section className="border border-teal/15 bg-[#080a0a] p-5"><div className="flex items-start gap-3"><div className="grid h-8 w-8 shrink-0 place-items-center border border-teal/25 font-mono text-[8px] text-teal">ST</div><div><p className="font-mono text-[8px] uppercase tracking-[0.16em] text-teal">Retrofit stress test</p><h2 className="mt-1 font-display text-2xl">Test the retrofit, not just the average.</h2><p className="mt-2 max-w-3xl text-[9px] leading-5 text-steel">Add a defensible proposed efficiency or envelope R-value and OVERHAUL will test the retrofit across explicit operating conditions. Regional weather can add a labeled stress boundary when a live site context is available. No stress case is treated as a measured forecast.</p></div></div></section>;
  }
  ...
  return <section className="overflow-hidden border border-teal/20 bg-[#080a0a] shadow-[0_20px_90px_rgba(0,0,0,.2)]">
    <div className="border-b border-steel/10 px-5 py-5">
      <div className="flex flex-wrap items-end justify-between gap-4"><div><p className="font-mono text-[8px] uppercase tracking-[0.18em] text-teal">Retrofit stress test</p><h2 className="mt-1 font-display text-3xl">Does the retrofit still work when conditions move?</h2><p className="mt-2 max-w-3xl text-[10px] leading-5 text-steel">A deliberate counterfactual sweep. For buildings, outdoor temperature changes around the supplied boundary. When resolved, the site’s live 7-day maximum is added as a separate contextual case. For equipment, operating load moves from 25% to 100% of rated capacity. The same deterministic model is rerun at every point.</p></div><span className="border border-teal/20 px-3 py-2 font-mono text-[7px] uppercase text-teal">{rows.length} explicit cases</span></div>
    </div>
    <div className="grid xl:grid-cols-[1.3fr_.7fr]">
      <div className="p-5">
        <div className="grid items-end gap-4" style={{ gridTemplateColumns: `repeat(${rows.length}, minmax(0,1fr))` }}>
          {rows.map((row) => <div key={row.label} className="min-w-0"><div className={`flex h-56 items-end justify-center gap-1.5 border-b border-steel/10 px-2 ${row.contextual ? "bg-gold/[0.045]" : "bg-[linear-gradient(to_top,rgba(255,255,255,.035),transparent)]"}`}><div title={`Current ${row.currentPower.toFixed(1)} kW`} className="w-1/3 min-w-[10px] bg-steel/35" style={{ height: `${Math.max(4, row.currentPower / maxPower * 100)}%` }}/><div title={`Retrofit ${row.retrofitPower.toFixed(1)} kW`} className="w-1/3 min-w-[10px] bg-teal/75" style={{ height: `${Math.max(4, row.retrofitPower / maxPower * 100)}%` }}/></div><p className={`mt-2 truncate text-center font-mono text-[7px] uppercase ${row.contextual ? "text-gold" : "text-steel"}`}>{row.label}</p></div>)}
        </div>
        <div className="mt-3 flex justify-center gap-5 font-mono text-[7px] uppercase text-steel"><span><i className="mr-2 inline-block h-2 w-2 bg-steel/35"/>Current</span><span><i className="mr-2 inline-block h-2 w-2 bg-teal/75"/>Retrofit</span><span className="text-gold">Regional context ≠ design baseline</span></div>
      </div>
