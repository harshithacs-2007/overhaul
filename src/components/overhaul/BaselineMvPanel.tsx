...
function validRecords(value: unknown): MVRecord[] {
  ...
  return value.filter((item): item is StoredRecord => Boolean(item && typeof item === "object"))
    .map((item, index) => ({
...
export default function BaselineMvPanel() {
  ...
  const [baselineTarget, setBaselineTarget] = useState("8+ intervals");
    ...
    <section className="mx-auto mt-4 max-w-[1500px] px-4 sm:px-6 lg:px-8">
      <div className="border border-steel/20 bg-black/15 p-5">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <p className="font-mono text-[9px] uppercase tracking-[0.18em] text-teal">Baseline + measurement & verification</p>
            <h2 className="font-display mt-1 text-3xl sm:text-4xl">Prove the retrofit, not just predict it.</h2>
            <p className="mt-2 max-w-4xl text-sm leading-6 text-steel">OVERHAUL calibrates an energy baseline against independently supplied operating drivers, adjusts that baseline for the post-retrofit conditions, then reports measured avoided energy with a model-error band.</p>
          </div>
        ...
        <div className="mt-5 grid gap-3 lg:grid-cols-4">
          <Contract title="1 · Baseline" value={baseline.status === "calibrated" ? "Locked calibration" : baselineTarget} note="energy + weather / occupancy / production" />
        ...
        {verification.status === "verified" ? (
          <div className="mt-4 border border-teal/25 bg-teal/5 p-4">
            <div className="grid gap-3 md:grid-cols-4">
              <Result label="Avoided energy" value={`${verification.avoidedKWh!.toFixed(1)} kWh`} note="adjusted baseline − measured post" />
              <Result label="Savings range" value={`${verification.savingsRangeKWh!.low.toFixed(1)} → ${verification.savingsRangeKWh!.high.toFixed(1)} kWh`} note="includes model-error band" />
              <Result label="Uncertainty" value={`±${verification.modelUncertaintyKWh!.toFixed(1)} kWh`} note="calibration-derived" />
              <Result label="Verdict" value={verification.avoidedKWh! >= 0 ? "positive" : "negative"} note="investigate before claiming success" />
            </div>
          </div>
        ) : null}
        ...
        {!records.length ? (
          <div className="mt-4 border border-steel/15 p-4 text-xs text-steel">
            No time-series M&V package is loaded yet. The contract is ready for interval energy data through <span className="font-mono text-paper">overhaul:mv-records</span>; OVERHAUL will not invent a baseline or savings number.
          </div>
        ) : null}
      </div>
    </section>
