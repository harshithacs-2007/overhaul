...
export default function RoomScanOverlay({ scope = "building" }: Props) {
  ...
  const [history, setHistory] = useState<Array<{ id: string; sector: number; image: string; result: ScanResult }>>([]);
  ...
  const sectorLabel = useMemo(() => `SECTOR ${String(sector + 1).padStart(2, "0")} / ${SECTORS}`, [sector]);
  ...
  const capture = async () => {
    ...
    if (!video || video.readyState < 2 || processing || history.length >= SECTORS) return;
    setProcessing(true);
    setScanning(true);
    ...
    try {
      ...
      form.append("file", new File([blob], `room-sector-${sector + 1}.jpg`, { type: "image/jpeg" }));
      ...
      setHistory((current) => [...current, { id: uid(), sector, image, result }]);
      if (sector < SECTORS - 1) {
        setSector((value) => value + 1);
        setRotationHint(`Turn about ${Math.round(360 / SECTORS)}° clockwise and capture the next sector.`);
      } else {
        setRotationHint("360° coverage complete. Review the evidence map before analysis.");
      }
    } catch (error) {
      setCameraError(error instanceof Error ? error.message : "Room scan failed.");
    } finally {
      setProcessing(false);
    }
  };
  ...
  if (!open) {
    return <>
      <button type="button" onClick={start} className="fixed bottom-5 left-5 z-40 border border-teal/40 bg-[#071313]/95 px-4 py-3 font-mono text-[9px] uppercase tracking-[0.14em] text-teal shadow-2xl backdrop-blur hover:border-teal sm:left-auto sm:right-[220px]">
        ◉ Room Scan · 360° CV
      </button>
      ...
      <div className="grid h-full lg:grid-cols-[1fr_360px]">
        <section className="relative min-h-0 overflow-hidden border-b border-steel/15 lg:border-b-0 lg:border-r">
          ...
          <div className="absolute left-0 right-0 top-0 flex items-center justify-between border-b border-white/10 bg-[#030606]/60 px-5 py-4 backdrop-blur-md">
            <div><p className="font-mono text-[8px] uppercase tracking-[0.18em] text-teal">OVERHAUL COMPUTER VISION</p><p className="mt-1 text-xs text-paper">Room intelligence scan</p></div>
          ...
          <div className="absolute bottom-6 left-1/2 w-[min(92%,760px)] -translate-x-1/2">
            ...
            <div className="flex items-center gap-3">
              <button type="button" onClick={capture} disabled={processing || history.length >= SECTORS} className="grid h-16 w-16 shrink-0 place-items-center rounded-full border-2 border-paper bg-paper/10 font-mono text-xs text-paper backdrop-blur-md disabled:opacity-40">{processing ? "…" : "SCAN"}</button>
              <div className="min-w-0 flex-1 border border-white/10 bg-[#030606]/75 px-4 py-3 backdrop-blur-md"><p className="text-[10px] text-paper">{rotationHint}</p><p className="mt-1 text-[8px] text-steel">Use all 12 sectors for a full visual sweep. OVERHAUL stores each view as evidence; it does not fabricate hidden geometry.</p></div>
            </div>
          </div>
        </section>
        ...
        <aside className="min-h-0 overflow-y-auto bg-[#070a0a]">
          ...
          <div className="space-y-4 p-5">
            ...
            </> : <div className="grid min-h-[320px] place-items-center border border-dashed border-steel/20 p-6 text-center"><div><div className="mx-auto h-12 w-12 rounded-full border border-teal/30"/><p className="mt-4 font-display text-lg">Point. Capture. Understand.</p><p className="mt-2 text-[9px] leading-5 text-steel">The first processed frame will populate this panel with bounded visual detections and evidence-grounded engineering clues.</p></div></div>}
            ...
            {history.length === SECTORS ? <motion.div initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} className="border border-teal/25 bg-teal/[0.05] p-4"><p className="font-mono text-[8px] uppercase text-teal">360° evidence bundle ready</p><p className="mt-2 text-[9px] leading-5 text-steel">The sweep is stored as multiple visual observations. Use it to seed the asset model and request only the measurements that can change the retrofit decision.</p></motion.div> : null}
          </div>
        </aside>
      </div>
    </div>
