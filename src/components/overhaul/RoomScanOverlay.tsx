...
const MAX_MS = 12000;
const RETRY_MS = 550;
const MAX_TICKS = Math.ceil(MAX_MS / RETRY_MS);
...
export default function RoomScanOverlay({ scope = "building" }: Props) {
  const router = useRouter(); const video = useRef<HTMLVideoElement>(null); const stream = useRef<MediaStream|null>(null); const timer = useRef<number|null>(null); const prev = useRef<ImageData|null>(null); const frames = useRef<Frame[]>([]); const sector = useRef(-1); const ticks = useRef(0);
  const [open,setOpen]=useState(false); const [holding,setHolding]=useState(false); const [processing,setProcessing]=useState(false); const [ready,setReady]=useState(false); const [error,setError]=useState<string|null>(null); const [status,setStatus]=useState("Ready to capture evidence."); const [progress,setProgress]=useState(0); const [coverage,setCoverage]=useState(0); const [twinStatus,setTwinStatus]=useState("pending"); const [lockedSectors,setLockedSectors]=useState<number[]>([]);

  const stopTimer=useCallback(()=>{ if(timer.current!=null) window.clearInterval(timer.current); timer.current=null; },[]);
  const close=useCallback(()=>{ stopTimer(); stream.current?.getTracks().forEach(t=>t.stop()); stream.current=null; setOpen(false); setHolding(false); setProcessing(false); },[stopTimer]);
  const start=useCallback(async()=>{
    setError(null); setReady(false); setCoverage(0); setTwinStatus("pending"); setLockedSectors([]);
    try {
  ...
  useEffect(()=>{ const openScan=()=>void start(); window.addEventListener("overhaul:open-room-scan",openScan); return()=>window.removeEventListener("overhaul:open-room-scan",openScan); },[start]);
  ...
  function capture(target:number): Frame|null {
    ...
    return {id:uid(),sector:target,quality:q,motion:m,image};
  }
  function begin(){
    if(!open||processing||holding) return; frames.current=[]; sector.current=-1; ticks.current=0; prev.current=null; setReady(false); setHolding(true); setProgress(0); setCoverage(0); setLockedSectors([]); setStatus("Calibrating camera… keep moving continuously.");
    const tick=()=>{
      ticks.current += 1; const elapsed=Math.min(MAX_MS,ticks.current*RETRY_MS); const target=Math.min(SECTORS-1,Math.floor((elapsed/MAX_MS)*SECTORS)); setProgress(Math.min(100,Math.round(elapsed/MAX_MS*100)));
      if(target!==sector.current){ const f=capture(target); if(f && f.quality>=MIN_QUALITY && (f.motion>=MIN_MOTION || target===0)){ frames.current=[...frames.current,f]; sector.current=target; setLockedSectors(prev=>prev.includes(target)?prev:[...prev,target]); setCoverage(Math.round(frames.current.length/SECTORS*100)); setStatus(`Locked sector ${target+1}/${SECTORS} · ${(f.quality*100).toFixed(0)}% frame quality`); } else setStatus(`Sector ${target+1}/${SECTORS} needs another view · keep moving.`); }
      if(ticks.current>=MAX_TICKS || frames.current.length===SECTORS){ stopTimer(); setHolding(false); void finish(); }
    };
  ...
  async function finish(){
    const captured=[...frames.current]; if(!captured.length){setStatus("No usable evidence captured. Improve lighting and move slower.");return;} setProcessing(true); setLockedSectors(captured.map(f=>f.sector)); setStatus(`Analysing ${captured.length} captured sectors…`);
    try{
