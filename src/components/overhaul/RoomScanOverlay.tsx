...
function motion(canvas: HTMLCanvasElement, prev: ImageData | null) {
...
export default function RoomScanOverlay({ scope = "building" }: Props) {
  ...
  function capture(target:number): Frame|null {
    ...
    const q=quality(c); const m=motion(c,prev.current); const sw=64, sh=Math.max(1,Math.round(c.height*sw/c.width)); prev.current=ctx.getImageData(0,0,sw,sh); const image=c.toDataURL("image/jpeg",.76);
  ...
  function begin(){
    ...
    const tick=()=>{
      const elapsed=performance.now()-started.current; const target=Math.min(SECTORS-1,Math.floor(elapsed/(MAX_MS/SECTORS))); current.current=target; setProgress(Math.min(100,Math.round(elapsed/MAX_MS*100))); setPulse((p)=>p+1);
  ...
  async function finish(){
    ...
    try{
      ...
      write("overhaul:room-scan",{scope,completed:complete,coveragePercent:Math.round(analysed.length/SECTORS*100),sectors:analysed.map(({image:_,...x})=>x),frameCount:analysed.length,updatedAt:new Date().toISOString()});
  ...
  return <AnimatePresence>{open&&<motion.div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/90 p-3 backdrop-blur-md" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}>
    <motion.div className="relative w-full max-w-6xl overflow-hidden border border-teal/30 bg-[#050909] shadow-2xl" initial={{scale:.96,y:18}} animate={{scale:1,y:0}}>
      <div className="grid lg:grid-cols-[1.35fr_.65fr]">
        ...
        <div className="flex flex-col p-5 sm:p-7"><button type="button" onClick={close} className="self-end font-mono text-[8px] uppercase text-steel hover:text-paper">Close</button><p className="mt-6 font-mono text-[8px] uppercase tracking-[.2em] text-teal">{scope} reconstruction</p><h2 className="mt-2 font-display text-4xl">Capture reality.<br/>Not assumptions.</h2><p className="mt-3 text-[10px] leading-5 text-steel">Weak views are retried instead of silently accepted. Geometry remains unverified until independent metric evidence supports scale.</p><div className="mt-6 grid grid-cols-6 gap-1.5">{Array.from({length:SECTORS},(_,i)=>{const locked=frames.current.some(f=>f.sector===i);return <motion.div key={i} className={`h-7 border ${locked?"border-teal/60 bg-teal/15":"border-steel/15 bg-white/[.02]"}`} animate={locked?{opacity:[.55,1,.55]}:{opacity:1}} transition={{duration:1.2,repeat:locked?Infinity:0}}/>})}</div><div className="mt-5 min-h-12 border border-steel/10 p-3 font-mono text-[8px] text-steel">{status}</div>{error&&<div className="mt-3 border border-red-300/25 bg-red-300/[.04] p-3 text-[9px] text-red-100">{error}</div>}<div className="mt-auto pt-6">{ready?<button type="button" onClick={()=>{close();router.push("/assessment");}} className="w-full border border-teal/50 bg-teal/10 px-5 py-4 font-mono text-[9px] uppercase tracking-[.12em] text-teal hover:bg-teal/15">Continue to evidence & assessment →</button>:<button type="button" disabled={processing||holding} onClick={begin} className="relative w-full overflow-hidden border border-teal/50 px-5 py-4 font-mono text-[9px] uppercase tracking-[.12em] text-teal disabled:opacity-40">{holding?`Scanning sector ${current.current+1}/${SECTORS}…`:processing?`Analysing evidence… · ${twinStatus}`:"Hold / press to scan 360°"}{holding&&<motion.span className="absolute inset-y-0 left-0 w-1/3 bg-teal/10" animate={{x:["-120%","420%"]}} transition={{duration:1.4,repeat:Infinity,ease:"linear"}}/>}</button>}</div></div>
      </div>
    </motion.div>
