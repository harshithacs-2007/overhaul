...
export default function MachineRetrofitMatrix({ assetClass, values }: { assetClass?: string | null; values: Values }) {
  ...
  useEffect(() => {
    ...
    setSelectedPathway((current) => next.some((p) => p.id === current) ? current : next[0]?.id || "");
  }, [machineClass]);
