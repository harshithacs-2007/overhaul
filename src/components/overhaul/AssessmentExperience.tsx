...
export default function AssessmentExperience() {
  ...
  const confidence = useMemo(() => { const list = extracts.flatMap((item) => item.observations || []).map((item) => item.confidence).filter((item) => Number.isFinite(item)); return list.length ? Math.round((list.reduce((sum, item) => sum + item, 0) / list.length) * 100) : roomScan?.completed ? 72 : 0; }, [extracts, roomScan]);
