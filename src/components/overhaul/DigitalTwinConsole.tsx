...
export default function DigitalTwinConsole({ scope, values }: Props) {
  ...
  useEffect(() => {
    ...
    window.addEventListener("storage", sync); window.addEventListener("overhaul:supplemental-change", sync);
    return () => { window.removeEventListener("storage", sync); window.removeEventListener("overhaul:supplemental-change", sync); };
  }, []);
  ...
  const observations = useMemo<ShadowObservation[]>(() => {
    ...
    for (const extraction of extracts) for (const observation of extraction.observations || []) {
      ...
      source.push({ key, value: observation.numericValue, unit: observation.unit || "", source: "vision", confidence: observation.confidence });
    }
  ...
  const expected = useMemo(() => buildEquipmentExpectations({
    ratedCapacityKW: Number(values.capacity_kw) > 0 ? Number(values.capacity_kw) : undefined,
    expectedEfficiency: Number(values.efficiency) > 0 ? Number(values.efficiency) : undefined,
  ...
  return <section className="mt-5 border border-steel/15 bg-[#070b0b]">
    ...
    <div className="grid gap-4 p-5 lg:grid-cols-[1.2fr_0.8fr]">
      <div className="space-y-2">{shadow.signals.length ? shadow.signals.map((signal) => <div key={signal.key} className="border border-steel/10 p-3"><div className="flex items-center justify-between gap-4"><div><p className="font-mono text-[8px] uppercase text-steel">{signal.key}</p><p className="mt-1 text-[11px]">{signal.observed.toFixed(2)} {signal.unit} <span className="text-steel">vs {signal.expected.toFixed(2)} {signal.unit}</span></p></div><span className={`font-mono text-[8px] uppercase ${signal.severity === "critical" ? "text-red-300" : signal.severity === "warning" ? "text-amber-200" : "text-teal"}`}>{signal.severity}</span></div><div className="mt-2 flex justify-between font-mono text-[7px] text-steel"><span>residual {(signal.relativeResidual * 100).toFixed(1)}%</span><span>{Math.round(signal.confidence * 100)}% source confidence</span></div></div>) : <div className="border border-dashed border-steel/20 p-6"><p className="font-display text-lg">No independently expected signal yet.</p><p className="mt-2 text-[9px] leading-5 text-steel">OVERHAUL will not call something inefficient merely because a model guessed it. Add a validated reference, operating measurement, or approved engineering baseline.</p></div>}</div>
      <aside className="border border-steel/10 p-4"><p className="font-mono text-[8px] uppercase text-steel">Decision-critical unknowns</p><div className="mt-3 space-y-2">{shadow.nextEvidence.length ? shadow.nextEvidence.map((item, i) => <div key={i} className="flex gap-2 text-[9px] leading-4 text-steel"><span className="text-teal">0{i + 1}</span><span>{item}</span></div>) : <p className="text-[9px] text-steel">No next measurement requested. The current evidence set is sufficient for the expected signals available.</p>}</div><div className="mt-5 grid grid-cols-2 gap-2"><Data label="Observations" value={String(shadow.observationsUsed)} /><Data label="Matched" value={String(shadow.expectedSignalsMatched)} /><Data label="Unknowns" value={String(shadow.unknowns.length)} /><Data label="Mode" value={scope} /></div></aside>
    </div>
  </section>;
}
