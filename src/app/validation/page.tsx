...
export default function ValidationPage() {
  ...
  const results = useMemo(() => {
    ...
    return {
      pump: [
        ["annual energy", pump.baseline.annualEnergyKWh, 105000],
        ["annual cost", pump.baseline.annualEnergyKWh * 9, 945000],
      ],
      retrofit: [
        ["baseline energy", retrofit.baseline.annualEnergyKWh, 105000],
        ["proposed energy", retrofit.proposed.annualEnergyKWh, 90000],
        ["energy saving", -retrofit.delta.annualEnergyKWh, 15000],
        ["cost saving", retrofit.delta.annualSavingINR, 135000],
      ],
      building: [
        ["baseline thermal load", building.baseline.thermalLoadKW, 33.3],
        ["baseline energy", building.baseline.annualEnergyKWh, 24975],
        ["proposed thermal load", building.proposed.thermalLoadKW, 29.1],
        ["proposed energy", building.proposed.annualEnergyKWh, 21825],
        ["energy saving", -building.delta.annualEnergyKWh, 3150],
      ],
  ...
  return <main className="min-h-screen bg-[#050707] text-paper">
    <div className="mx-auto max-w-7xl px-5 py-8 sm:px-8">
      ...
      <section className="mt-6 overflow-hidden border border-steel/15">
        <div className="flex items-center justify-between border-b border-steel/10 p-5"><div><p className="font-mono text-[8px] uppercase text-teal">01 // Physics regression</p><h2 className="mt-1 font-display text-3xl">Known input → known consequence</h2></div><button type="button" onClick={() => setRan((x) => !x)} className="border border-teal/30 px-4 py-2 font-mono text-[8px] uppercase text-teal">Re-run</button></div>
      ...
      <section className="mt-6 border border-steel/15 p-5">
        <p className="font-mono text-[8px] uppercase text-teal">02 // Numeric input regression</p><h2 className="mt-1 font-display text-3xl">Human input should not become NaN</h2><p className="mt-2 text-[10px] leading-5 text-steel">This parser accepts ordinary formatted values such as 1,200 while rejecting malformed input. Production UI must apply the same normalization before physics functions receive values.</p>
