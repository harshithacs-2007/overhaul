...
export async function GET(request: Request) {
  ...
  if (latitude == null || longitude == null || latitude < -90 || latitude > 90 || longitude < -180 || longitude > 180 || !Number.isInteger(year) || year < 2001 || year > new Date().getUTCFullYear() - 1) return NextResponse.json({ error: "Valid coordinates and a complete historical year are required." }, { status: 400 });

  const endpoint = new URL("https://power.larc.nasa.gov/api/temporal/hourly/point");
  endpoint.searchParams.set("parameters", "T2M,RH2M,PS,ALLSKY_SFC_SW_DWN");
  ...
  try {
    ...
    const payload = await response.json() as { properties?: { parameter?: { T2M?: Record<string, number>; RH2M?: Record<string, number>; PS?: Record<string, number>; ALLSKY_SFC_SW_DWN?: Record<string, number> } } };
    const temperatures = payload.properties?.parameter?.T2M || {};
    const humidity = payload.properties?.parameter?.RH2M || {};
    const pressure = payload.properties?.parameter?.PS || {};
    ...
    const series = timestamps.map((timestamp) => {
      const outdoorTempC = Number(temperatures[timestamp]);
      const outdoorRHPercent = Number(humidity[timestamp]);
      const surfacePressureKPa = Number(pressure[timestamp]);
      const solarKWhM2 = Number(solar[timestamp]);
      if (!Number.isFinite(outdoorTempC) || !Number.isFinite(outdoorRHPercent) || !Number.isFinite(surfacePressureKPa)) return null;
      return { timestamp, outdoorTempC, outdoorRHPercent: Math.max(0, Math.min(100, outdoorRHPercent)), surfacePressureKPa, solarIrradianceKWhM2: Number.isFinite(solarKWhM2) && solarKWhM2 >= 0 ? solarKWhM2 : 0 };
    }).filter((item): item is { timestamp: string; outdoorTempC: number; outdoorRHPercent: number; surfacePressureKPa: number; solarIrradianceKWhM2: number } => Boolean(item));
