...
The learned model is a screening/cross-check layer only. OVERHAUL's deterministic
engineering core remains authoritative for retrofit consequences and decisions.

Input:  house_features_rescast-100k.parquet (or another RESCAST-compatible table)
...
def split_frame(df: pd.DataFrame) -> tuple[np.ndarray, np.ndarray, np.ndarray, str]:
    ...
    train[train_idx] = True; val[val_idx] = True; test[test_idx] = True
...
def prepare_matrix(df: pd.DataFrame, features: list[str], train_mask: np.ndarray):
    ...
    return standardized, missing_fraction, means.tolist(), scale.tolist()
...
def predict(X: np.ndarray, intercept: float, coefficients: np.ndarray):
...
def target_record(df, requested_target, X, missing_fraction, train, val, test, means, scale):
    ...
    for lam in RIDGE_LAMBDAS:
        ...
        if score < best_score:
            best_score = score; best = (lam, intercept, coefficients)
    ...
    test_metrics = metrics(y_raw[test_mask], test_pred)
    ...
    return {
        ...
        "targetMean": float(np.mean(y_raw[train_mask])),
        "targetStd": float(np.std(y_raw[train_mask]) or 1.0),
...
def main():
    ...
    if len(df) > args.sample:
        df = df.sample(args.sample, random_state=SEED).reset_index(drop=True); sampled = True

    features = choose_features(df, FEATURES)
    if not features: raise SystemExit("No conservative quantitative RESCAST features were found.")
    train, val, test, split_method = split_frame(df)
    X, missing_fraction, feature_mean, feature_scale = prepare_matrix(df, features, train)
    records = {
        "schemaVersion": "1.1",
        "trainingDataset": args.input.name,
        "trainingDatasetSha256": dataset_hash,
        "sourceRowsSampled": int(len(df)),
        "sampledFromLargerFile": sampled,
